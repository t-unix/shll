function __bobthefish_colors -S -a color_scheme -d 'Define colors used by bobthefish'
  switch "$color_scheme"
    ...
    case 'terminal' 'insh*'
      set -l colorfg black
      [ "$color_scheme" = 'insh' ]; and set colorfg white
      ...
      set -x color_path                     4E4E4E white
      set -x color_path_basename            4E4E4E white --bold
      set -x color_path_nowrite             magenta $colorfg
      set -x color_path_nowrite_basename    magenta $colorfg --bold

      set -x color_repo                     2e2e2e white
      set -x color_repo_work_tree           black $colorfg --bold
      set -x color_repo_dirty               brred $colorfg
      set -x color_repo_staged              2e2e2e yellow

      set -x color_vi_mode_default          0e0e0e 404040 --bold
      set -x color_vi_mode_insert           0e0e0e 404040 --bold
      set -x color_vi_mode_visual           0e0e0e 404040 --bold

      set -x color_vagrant                  brcyan $colorfg
      set -x color_k8s                      (__c_bg $KUBECONFIG) (__c_fg $KUBECONFIG) --bold
      set -x color_username                 (__c_bg $hostname) (__c_fg $hostname) --bold
      set -x color_hostname                 (__c_bg $hostname) (__c_fg $hostname)
    ...
    case '*' # default dark theme
      ...
      set -x color_repo                     $white $green[3]
